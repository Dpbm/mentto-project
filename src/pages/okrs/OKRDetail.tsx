...
const OKRDetail = () => {
	...
	const calculateProgress = (data: OKRFormData) => {
		let total = 0;
		let current = 0;

		for (const result of data.keyResults) {
			total += parseInt(result.target || '0');
		...
		const progress = Number(((current / total) * 100).toFixed());
		return progress;
	};
	...
	const onSubmit = async (data: OKRFormData) => {
		...
		try {
			...
			const partialData = {
				...
				year: data.year,
				status:
					progress >= 100 && data.status == 'active'
						? 'completed'
						: data.status,
				progress,
			};

			form.setValue('status', partialData.status);

			const { error } = await supabase
				.from('okrs')
				.update({ ...partialData, key_results: data.keyResults })
				.eq('id', id);
